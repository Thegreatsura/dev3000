...
export const DEEPSEC_DEV_AGENT_ACTION_STEPS: DevAgentActionStep[] = [
  ...
  },
  {
    ...
    config: {
      prompt:
        "Install the .deepsec workspace dependencies, run `timeout 90s node node_modules/@anthropic-ai/claude-code/install.cjs` from `.deepsec/` so the postinstall cannot hang indefinitely, verify `node_modules/.bin/claude --version`, read DeepSec's installed SKILL.md and SETUP.md, and replace INFO.md with concise project-specific context. If the timeout command is unavailable, use an equivalent Node child_process timeout wrapper; do not run install.cjs unbounded."
    }
  },
  {
    kind: "send-prompt",
