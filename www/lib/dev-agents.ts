...
export const DEEPSEC_DEV_AGENT_ACTION_STEPS: DevAgentActionStep[] = [
  ...
  },
  {
    ...
    config: {
      prompt:
        "Install the .deepsec workspace dependencies, ensure the Claude Agent SDK native binary that DeepSec uses is present, read DeepSec's installed SKILL.md and SETUP.md, and replace INFO.md with concise project-specific context. Do not try to run a Claude Code postinstall; DeepSec uses @anthropic-ai/claude-agent-sdk."
    }
  },
  {
    kind: "send-prompt",
